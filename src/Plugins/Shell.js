+(function (window, document, undefined) {
	$.fn.shell = function (config) {
		...
		var cursor = '<span class="yaex-shell-cursor">&nbsp;</span>';

		////////////////////////////////////////////////////////////////////////
		// Globals
		var container = $(this);
		var inner = $('<div class="yaex-shell-inner"></div>');
		// erjiang: changed this from a text input to a textarea so we
		// can get pasted newlines
		var typer = $('<textarea class="yaex-shell-typer"></textarea>');
		// Prompt
		var promptBox;
		var prompt;
		...
		(function () {
			...
			if (config.welcomeMessage)
				message(config.welcomeMessage, 'yaex-shell-welcome');
			newPromptBox();
			if (config.autofocus) {
				inner.addClass('yaex-shell-focus');
				typer.focus();
				setTimeout(function () {
					inner.addClass('yaex-shell-focus');
					typer.focus();
		...
		function newPromptBox() {
			...
			enableInput();
			promptBox = $('<div class="yaex-shell-prompt-box"></div>');
			var label = $('<span class="yaex-shell-prompt-label"></span>');
			var labelText = extern.continuedPrompt ? continuedPromptLabel : promptLabel;
			promptBox.append(label.text(labelText).show());
			label.html(label.html().replace(' ', '&nbsp;'));
			prompt = $('<span class="yaex-shell-prompt"></span>');
		...
		container.click(function () {
			inner.addClass('yaex-shell-focus');
			inner.removeClass('yaex-shell-nofocus');
			typer.focus();
			scrollToBottom();
			return false;
		});
		...
		// Handle losing focus
		typer.blur(function () {
			inner.removeClass('yaex-shell-focus');
			inner.addClass('yaex-shell-nofocus');
		});
		...
		typer.keypress(function (e) {
			...
			if (acceptInput && cancelKeyPress != keyCode && keyCode >= 32) {
				...
				if (typeof config.charInsertTrigger == 'undefined' ||
					...
					typer.shellInsert(keyCode);
			}
			if ($.Yaex.Browser.webkit) return false;
		});
		...
		function commandTrigger() {
			...
			if (typeof config.commandValidate == 'function') {
				...
				if (ret == true || ret == false) {
					...
					commandResult(ret, "yaex-shell-message-error");
				}
			} else {
				handleCommand();
			}
			...
			if (typeof config.commandHandle == 'function') {
				...
				if (typeof ret == 'boolean') {
					if (ret) {
						...
						commandResult('Command failed.', "yaex-shell-message-error");
					}
				} else if (typeof ret == "string") {
					commandResult(ret, "yaex-shell-message-success");
		...
		function message(msg, className) {
			var mesg = $('<div class="yaex-shell-message"></div>');
		...
		typer.shellInsert = function (data) {
